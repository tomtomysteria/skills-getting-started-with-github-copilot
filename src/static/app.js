    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0 
          ? details.participants.map(p => `<li><span class="participant-email">${p}</span><span class="delete-icon" data-activity="${name}" data-email="${p}" title="Unregister">🗑️</span></li>`).join('')
